#!/usr/bin/env perl
# downloaded from https://www.gringene.org/code/perlshaper.pl
# by Wikipedia/User:gringer https://en.wikipedia.org/wiki/User:gringer?rdfrom=commons:User:Gringer
# obtain data from https://www.naturalearthdata.com/
use warnings;
use strict;
...
=item B<-annotate> I<Text>;I<lat>,I<long>
...
if(!exists($projOpts->{"resolution"})){
  $projOpts->{"resolution"} = 1;
}
...
if(!exists($projOpts->{"mapType"})){
  $projOpts->{"mapType"} = "<undefined>";
}
...
$projOpts->{"xScale"} = 1 unless $projOpts->{"xScale"};
$projOpts->{"yScale"} = 1 unless $projOpts->{"yScale"};
$projOpts->{"xAdj"} = $projOpts->{"svgWidth"} / 2;
$projOpts->{"yAdj"} = $projOpts->{"svgHeight"} / 2;
...
if ($projOpts->{"zoomed"}) {
  ...
  if($projOpts->{"manualZoom"}){
    ...
    $projOpts->{"centreLn"} =
	($projOpts->{"minX"} + $projOpts->{"maxX"}) / 2;
    $projOpts->{"centreLt"} =
	($projOpts->{"minY"} + $projOpts->{"maxY"}) / 2;
...
my $svg = SVG->new('height' => $projOpts->{"svgHeight"} + $projOpts->{"padding"} * 2,
                   'width' => $projOpts->{"svgWidth"} + $projOpts->{"padding"} * 2,
...
if(keys(%annotations)){
  ...
  while(my($text, $pos) = each %annotations){
    ...
    my @tmpPos = ([$F[1],$F[0]], [$F[1],$F[0]]);
    my @projPos = project($projOpts, \@tmpPos);
    if($projPos[0] == 0){
      print(STDERR "[Point not on map, ignoring]\n");
      next;
    }
